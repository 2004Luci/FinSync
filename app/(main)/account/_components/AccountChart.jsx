    ...
    return (
        <Card className='bg-background_secondary border-util_color'>
            <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-7'>
                <CardTitle className='text-base font-normal gradient-text-normal'>Transaction Overview</CardTitle>
                <Select defaultValue={dateRange} onValueChange={setDateRange}>
                    <SelectTrigger className="w-[140px] text-subtext border-util_color">
                        <SelectValue placeholder="Select range" />
                    </SelectTrigger>
                    <SelectContent className='text-main bg-background_secondary'>
                        {Object.entries(DATE_RANGES).map(([key, { label }]) => {
                            return (<SelectItem key={key} value={key} className='cursor-pointer hover:bg-main hover:text-background'>{label}</SelectItem>);
                        })}
                    </SelectContent>
                </Select>
            </CardHeader>
                ...
                <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            ...
                            <XAxis dataKey="date" />
                            <YAxis fontSize={12} fontWeight={800} tickLine={false} axisLine={false} tickFormatter={(value) => `₹${value}`} />
